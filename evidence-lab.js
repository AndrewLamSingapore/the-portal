(function () {
  ...
  function render(report) {
    ...
    byId('checkRows').innerHTML = report.checks.map(item => `<tr><td>${escape(item.field)}<br><small>${escape(item.reason)}</small></td><td class="result-${item.result.toLowerCase()}">${escape(item.result)}</td><td>${escape(item.expected)}</td><td>${escape(item.observed)}</td></tr>`).join('');
