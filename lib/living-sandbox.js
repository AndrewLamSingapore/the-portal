...
function makeFossilRecord(population = []) {
  return population.filter(candidate => candidate.state === 'DEAD').map(candidate => ({
    id: candidate.id,
    statement: candidate.statement,
    final_confidence: candidate.confidence,
    ancestry: clone(candidate.ancestry),
    falsifiers: clone(candidate.falsifiers),
...
export function runLivingSandbox(seed = SANDBOX_SEED) {
  ...
  const fossilRecord = makeFossilRecord(population);
  ...
  return {
    ...
    acceptance: {
      ...
      original_killed: degradation.state === 'DEAD',
      dead_retained_in_population: population.some(item => item.id === degradation.id && item.state === 'DEAD'),
      dead_retained_in_fossil_record: fossilRecord.some(item => item.id === degradation.id),
