...
export function applyExperimentalSelection(candidate, outcome, generation) {
  ...
  const declaredFalsifierObserved = outcome.polarity === 'CONTRADICTS'
    && Boolean(candidate.falsifiers?.includes(outcome.observation));
  const state = declaredFalsifierObserved || confidence <= .12
    ? 'DEAD'
    : outcome.polarity === 'CONTRADICTS' && confidence < .4
      ? 'CONTESTED'
      : confidence >= .55
        ? 'LIVING'
        : candidate.state;
