...
const SCHEMA = {
  ...
  required: ['era', 'year', 'type', 'title', 'description', 'provenance', 'condition', 'imagined_future', 'problem', 'status', 'modern_descendant', 'concepts', 'question', 'evidence_level', 'sources', 'relationships', 'experiment', 'connections']
};
...
async function requestArtifact({ mode, context, signal, fetchImpl }) {
  const upstream = await fetchImpl('https://api.openai.com/v1/responses', {
    ...
    body: JSON.stringify({
      model: MODEL,
      instructions: SYSTEM,
      input: `CURATOR MODE: ${mode.toUpperCase()}\n${MODES[mode]}\nCreate a small, safe, testable real-world experiment. connections may target only exact RECENT ARTIFACT IDs supplied below; use [] if none genuinely fits.${context}`,
      max_output_tokens: MAX_OUTPUT_TOKENS,
      text: { format: { type: 'json_schema', name: 'portal_artifact_v5', strict: true, schema: SCHEMA } }
    })
  });
