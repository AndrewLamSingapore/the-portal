...
const MODEL = process.env.PORTAL_LIVING_MODEL || 'gpt-5.6';

const SYSTEM = `You are the generative cortex inside The Portal ${PRODUCT_VERSION} evolutionary sandbox. Generate competing possibilities from supplied observations without pretending speculation is fact. Seek non-obvious but defensible explanations. Every candidate must expose ancestry, assumptions, prospective predictions, concrete falsifiers and explicit hard-kill conditions. Include at least one serious alternative explanation. Explorer candidates may be highly speculative, including counterfactual or unknown-law possibilities, but must label that epistemic class accurately. Scientist candidates prioritize evidence and discriminating tests. Skeptic candidates attack attractive explanations and search for confounders. Never fabricate sources. Return only structured data.`;
...
const CANDIDATE = { type: 'object', additionalProperties: false, properties: {
  ...
  hard_kill_conditions: { type: 'array', items: { type: 'string' }, minItems: 1, maxItems: 4 },
  discriminating_test: { type: 'string' }, confidence: { type: 'number', minimum: 0.05, maximum: 0.8 }, fitness: FITNESS
}, required: ['statement','niche','epistemic_class','parent_ids','assumptions','predictions','falsifiers','hard_kill_conditions','discriminating_test','confidence','fitness'] };
...
export async function generateHypothesisPopulation({ observations, memory = [], generation = 1, fetchImpl = fetch }) {
  ...
  const parentIds = new Set([...observations, ...memory].map(item => item?.id).filter(Boolean));
  const upstream = await fetchImpl('https://api.openai.com/v1/responses', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${process.env.OPENAI_API_KEY}` }, body: JSON.stringify({
    ...
    input: `GENERATION ${generation}\nOBSERVATIONS:\n${JSON.stringify(observations)}\nRELEVANT MEMORY:\n${JSON.stringify(memory)}\nGenerate a diverse population. The population MUST contain at least one EXPLORER, at least one SCIENTIST and at least one SKEPTIC candidate, with a different statement for every candidate. Do not assume known physics is the full search boundary, but do not label unknown-law speculation as evidence. parent_ids must use exact observation or memory IDs supplied above.`,
  ...
  const candidates = parsed.candidates.map((candidate, index) => {
    const invalidParents = candidate.parent_ids.filter(id => !parentIds.has(id));
    if (invalidParents.length) throw new Error(`Generated ancestry references unknown parent IDs: ${invalidParents.join(', ')}`);
    return {
      ...
      predictions: candidate.predictions, falsifiers: candidate.falsifiers, hard_kill_conditions: candidate.hard_kill_conditions,
      discriminating_test: candidate.discriminating_test, fitness: candidate.fitness, history: []
    };
  });
  return { ...parsed, candidates, model: MODEL };
}
