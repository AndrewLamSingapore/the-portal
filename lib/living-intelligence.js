...
export function chooseTools({ goal = '', uncertainty = .5, contradictions = 0, memoryHits = 0, populationCount = 0 } = {}) {
  const selected = [];
  if (memoryHits === 0) selected.push('memory.search');
  if (populationCount > 1 || uncertainty >= .45) selected.push('evidence.compare');
...
export function runExecutiveCycle({ goal, population, memory, history = [], noveltyBenchmark = null }) {
  const query = goal.toLowerCase().split(/\W+/).filter(term => term.length > 4);
  const evidencePopulation = [...history, ...population];
  const contradictions = evidencePopulation.reduce((n, hypothesis) => n + (hypothesis.evidence || []).filter(evidence => evidence.polarity === 'CONTRADICTS').length, 0);
  const uncertainty = 1 - Math.max(...population.map(hypothesis => hypothesis.confidence || 0), 0);
  const selectedTools = chooseTools({ goal, uncertainty, contradictions, memoryHits: 0, populationCount: evidencePopulation.length });
  const toolTrace = executeToolPlan({ tools: selectedTools, goal, query, population: evidencePopulation, memory });
  ...
  const displaced = evidencePopulation.find(candidate => candidate.superseded_by === winner?.id)
    || evidencePopulation.find(candidate => candidate.id !== winner?.id && ['DEAD', 'CONTESTED'].includes(candidate.state))
  ...
  const acceptance = {
    ...
    unprompted_open_ended_novelty_proven: noveltyBenchmark?.passed === true
  };
  const scientificAcceptancePassed = Object.values(acceptance).every(Boolean);

  return {
    goal,
    readiness: scientificAcceptancePassed ? 'SCIENTIFIC_ACCEPTANCE_READY' : 'SANDBOX_FOUNDATION_READY',
    decisive_gate: scientificAcceptancePassed ? 'SCIENTIFIC_ACCEPTANCE_PASSED' : 'OPEN_ENDED_NOVELTY_BENCHMARK_PENDING',
    metacognition: {
