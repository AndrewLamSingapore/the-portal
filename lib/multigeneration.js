...
export function reproducePopulation(population = [], generation = 2) {
  ...
  for (let i = 0; i < frontier.length; i += 1) {
    ...
    children.push({
      id: `HYP-G${generation}-${String(i + 1).padStart(2,'0')}`,
      generation,
      ...
      niche,
      epistemic_class: niche === 'EXPLORER' && a.epistemic_class === 'INFERRED' ? 'EXTRAPOLATED' : a.epistemic_class,
      state: 'EMERGING',
      confidence: clamp((a.confidence + b.confidence) / 2 * .92),
      ancestry: { parent_ids: crossover ? [a.id, b.id] : [a.id], operators: [crossover ? 'CROSSOVER' : 'MUTATION'] },
      evidence: [],
      assumptions: [...new Set([...(a.assumptions || []), ...(b.assumptions || [])])].slice(0,5),
      predictions: [...new Set([...(a.predictions || []), ...(b.predictions || [])])].slice(0,5),
      falsifiers: [...new Set([...(a.falsifiers || []), ...(b.falsifiers || [])])].slice(0,5),
      hard_kill_conditions: [...new Set([...(a.hard_kill_conditions || []), ...(b.hard_kill_conditions || [])])].slice(0,4),
      ...
      fitness: mutateFitness(meanFitness(a.fitness, b.fitness), niche, generation),
      history: []
    });
  }
  return children;
