...
export default async function handler(req, res) {
  ...
  const initialPopulation = generated?.candidates?.length
    ? generated.candidates
    : result.population.map((candidate, index) => ({ ...candidate, id: `HYP-G1-${String(index + 1).padStart(2, '0')}`, generation: 1 }));
  ...
  for (const hypothesis of finalPopulation) {
    memory.remember({ id: `MEM-LINEAGE-${hypothesis.id}`, layer: 'EPISTEMIC', content: { hypothesis: hypothesis.id, ancestry: hypothesis.ancestry, fitness: hypothesis.fitness } });
  }
  const cognition = runExecutiveCycle({
    goal: 'Challenge the surviving multi-generation possibilities and choose the next discriminating observation.',
  ...
  return res.status(200).json({
    product: 'The Portal',
    version: '6.3.0-preview.6',
    mode: 'SANDBOX',
    generation_mode: generated ? 'MODEL_ORIGINATED' : 'DETERMINISTIC_FALLBACK',
    generation_model: generated?.model || null,
    generation_error,
    safety: {
