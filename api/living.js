...
export default async function handler(req,res){
  ...
  try{generated=await generateHypothesisPopulation({observations:result.observations,memory:memory.records,generation:1});}catch(e){generation_error=String(e?.message||'generation failed').slice(0,240);}
  ...
  return res.status(200).json({
    product:'The Portal',version:'6.3.0-preview.8',mode:'SANDBOX',
    generation_mode:generated?'MODEL_ORIGINATED':'DETERMINISTIC_FALLBACK',generation_model:generated?.model||null,generation_error,
