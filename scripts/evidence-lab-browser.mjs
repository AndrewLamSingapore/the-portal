...
const fixture = {
  ...
  eth_getTransactionByHash:{ hash:txHash,type:'0x2',chainId:'0xaa36a7',from:`0x${'a'.repeat(40)}`,to:`0x${'b'.repeat(40)}`,nonce:'0x1',value:'0x0',input:'0x',gas:'0x5208',maxFeePerGas:'0x64',maxPriorityFeePerGas:'0xa',blockHash,blockNumber:'0x10',transactionIndex:'0x0' },
  eth_getTransactionReceipt:{ transactionHash:txHash,blockHash,blockNumber:'0x10',status:'0x1',type:'0x2',from:`0x${'a'.repeat(40)}`,to:`0x${'b'.repeat(40)}`,gasUsed:'0x5208',effectiveGasPrice:'0x32' },
...
function bridgeResponse(res) {
  return { setHeader:(key,value)=>res.setHeader(key,value), status(code){res.statusCode=code;return this;}, json(body){res.end(JSON.stringify(body));return this;} };
}
...
const server = http.createServer(async (req,res) => {
  if (req.url === '/api/evidence-lab' && req.method === 'POST') {
    ...
    global.fetch=async(_url,options)=>{const {method}=JSON.parse(options.body);return {ok:true,status:200,text:async()=>JSON.stringify({jsonrpc:'2.0',id:1,result:fixture[method]})};};
    try { await handler({method:'POST',headers:req.headers,socket:req.socket,body:JSON.parse(raw)},bridgeResponse(res)); }
    finally { global.fetch=originalFetch; }
    return;
  }
  const relative=req.url==='/'?'evidence-lab.html':req.url.replace(/^\//,'').replace(/\?.*$/,'');
  const file=path.join(root,relative); if(!file.startsWith(root)||!fs.existsSync(file)){res.statusCode=404;res.end('Not found');return;}
  const types={'.html':'text/html','.js':'text/javascript','.css':'text/css'};res.setHeader('content-type',types[path.extname(file)]||'application/octet-stream');res.end(fs.readFileSync(file));
});
