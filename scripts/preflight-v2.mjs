...
const inhabitantsHtml = fs.readFileSync('inhabitants.html', 'utf8');
const inhabitantsJs = fs.readFileSync('inhabitants.js', 'utf8');
for (const token of ['MEET THE', 'INHABITANTS', 'FRIENDLY, NOT AUTHORITATIVE', 'GROW THROUGH EVIDENCE', 'REMEMBER WITHOUT PUNISHMENT', 'KEEP AUTHORITY OUTSIDE']) {
  assert.ok(inhabitantsHtml.includes(token), `missing Living Inhabitants contract: ${token}`);
}
for (const token of ["fetch('/api/living'", 'Seed', 'Hatchling', 'Explorer', 'Researcher', 'Specialist', 'Sage', 'FEED KNOWLEDGE', 'REAL-WORLD AUTHORITY BLOCKED']) {
  assert.ok(inhabitantsJs.includes(token), `missing Living Inhabitants behavior: ${token}`);
}
assert.ok(!inhabitantsHtml.includes('Tamagotchi'), 'public identity must remain original');

console.log('PASS: complete living knowledge graph and Living Inhabitants release preflight.');
