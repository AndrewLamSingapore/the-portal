...
const publicRoutes = ['artifact', 'artifact-v1', 'archive', 'capabilities', 'evidence', 'evidence-lab', 'exhibition', 'exhibitions', 'graph', 'health', 'manifest', 'metrics', 'readiness', 'relationship', 'serendipity', 'status', 'trial', 'v2', 'verify', 'version'];

for (const route of publicRoutes) {
  assert.ok(directRoutes.has(route) || rewrittenRoutes.has(route), `missing public route /api/${route}`);
}
assert.ok(graph.includes("setHeader('Cache-Control','no-store')"), 'the living graph must never trail a newly persisted encounter');
