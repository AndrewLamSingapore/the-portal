...
async function get(path) {
  const response = await fetch(base + path, {
    headers: { 'user-agent': `portal-smoke/${PRODUCT_VERSION}`, 'cache-control': 'no-cache' },
...
const [appResult, styleResult, motionStyleResult, motionResult, livingPageResult, livingResult, versionResult, healthResult, statusResult, readinessResult, manifestResult] = await Promise.all([
  get('/app.js'),
  get('/styles.css'),
  get('/motion.css'),
  get('/motion.js'),
...
assert.equal(living.version, PRODUCT_VERSION);
...
assert.equal(version.version, PRODUCT_VERSION);
...
assert.equal(health.product_version, PRODUCT_VERSION);
...
assert.equal(status.product_version, PRODUCT_VERSION);
const readiness = json(readinessResult, '/api/readiness');
assert.equal(readinessResult.response.status, 200);
assert.equal(readiness.ok, true);
assert.equal(readiness.checks.experiment_result_schema, true);
assert.equal(readiness.checks.authenticated_result_writing, true);
...
console.log(`PASS: Portal ${PRODUCT_VERSION} production verified end to end (${archive.count} objects, revision ${health.revision || 'unknown'}).`);
